{{-- ============================================================
     STUDENT LAYOUT
     ============================================================
     Sidebar  : navigation only (no user card, no logout, no messages)
     Navbar   : KinderLearn logo · hamburger · messages · bell · avatar
     Avatar   : dropdown with My Profile / Settings / Logout
     ============================================================ --}}
@extends('layouts.app')
...
<div class="flex h-screen overflow-hidden" style="background-color: #f5f7fb;">
    ...
    <aside id="main-sidebar"
        ...
        <nav class="flex flex-col gap-0.5 flex-1 px-2 overflow-y-auto pb-4 pt-4">
            ...
            <a href="{{ route('student.modules') }}"
               ...
               class="sb-link {{ request()->routeIs('student.modules*') ? 'sb-active' : '' }}">
            ...
            <a href="{{ route('student.activities') }}"
               ...
               class="sb-link {{ request()->routeIs('student.activities*') ? 'sb-active' : '' }}">
    ...
    <div id="content-wrapper" class="flex flex-col w-full overflow-hidden">
        ...
        {{-- ============================================================
             TOP NAVBAR
             LEFT  : KinderLearn logo  +  hamburger
             RIGHT : messages icon  +  notifications  +  avatar dropdown
             z-40 so dropdown floats above the sidebar (z-30)
             ============================================================ --}}
        <header class="fixed top-0 left-0 right-0 z-40 bg-white flex items-center justify-between px-5 py-3"
            ...
            <div class="flex items-center gap-3">
                ...
                {{-- Hamburger — mobile only; desktop uses hover --}}
...
<div id="support-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
    <div class="bg-white rounded-3xl shadow-2xl w-full mx-4 p-7" style="max-width:440px;">
        ...
        <form method="POST" action="{{ route('student.support.send') }}" class="space-y-4">
            @csrf
            <div>
...
<script>
(function () {
    var CONVOS_URL  = '{{ route("student.messages.conversations") }}';
    var THREAD_BASE = '{{ url("/student/messages/thread") }}/';
    var SEARCH_URL  = '{{ route("student.messages.search") }}';
    var SEND_URL    = '{{ route("student.messages.send") }}';
    var CSRF        = '{{ csrf_token() }}';
...
<script>
(function () {
    var badges  = @json(session('new_badges'));
    var current = 0;
