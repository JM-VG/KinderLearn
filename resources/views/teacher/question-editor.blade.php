...
@section('title', 'Edit Questions — ' . $activity->title)
...
<div class="flex items-center gap-4 mb-6">
    <a href="{{ route('teacher.modules.edit', $activity->module_id) }}"
    ...
    <div>
        ...
        <p class="text-sm text-gray-400 font-semibold mt-0.5">{{ $activity->title }}</p>
    </div>
</div>
...
<form method="POST" action="{{ route('teacher.activities.questions.save', $activity->id) }}"
...
<div class="q-card" data-index="{{ $i }}">
    ...
    <div class="mb-3">
        ...
        <input type="hidden" name="questions[{{ $i }}][image]" value="{{ $q['image'] ?? '' }}">
        @if(!empty($q['image']))
        <img src="{{ asset('storage/' . $q['image']) }}" class="img-preview mb-2" id="img-preview-{{ $i }}">
        @else
        ...
        <label class="flex items-center gap-2 text-xs font-semibold text-indigo-600 cursor-pointer hover:underline">
            <i class="fa-solid fa-upload"></i> {{ empty($q['image']) ? 'Upload image' : 'Replace image' }}
