    ...
    <div class="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12">
        ...
        <div class="flex-1 grid grid-cols-2 gap-4" id="subjects">
            ...
            <div class="card-hover rounded-3xl p-6 text-center shadow-md cursor-pointer"
                 style="background: {{ $card['bg'] }}; border: 3px solid {{ $card['color'] }}30;">
                <div class="mb-3" style="font-size:3rem; color:{{ $card['color'] }};"><i class="{{ $card['fa'] }}"></i></div>
                <div class="font-fredoka text-xl" style="color: {{ $card['color'] }}">{{ $card['title'] }}</div>
...
<section class="py-12 px-6" style="background: linear-gradient(135deg, #FF6B6B, #FF8E53);">
    <div class="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 text-center text-white">
        ...
        <div class="bg-white bg-opacity-20 rounded-3xl p-6">
            <div class="mb-2" style="font-size:2.5rem;"><i class="{{ $stat['fa'] }}"></i></div>
            <div class="font-fredoka text-4xl mb-1">{{ $stat['value'] }}</div>
            <div class="font-semibold opacity-90">{{ $stat['label'] }}</div>
        </div>
        @endforeach
    </div>
</section>
    ...
    <div class="max-w-5xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="card-hover bg-gray-50 rounded-3xl p-8 text-center">
                <div class="mb-4" style="font-size:3rem; color:{{ $f['color'] }};"><i class="{{ $f['fa'] }}"></i></div>
                <h3 class="font-fredoka text-2xl mb-3" style="color: {{ $f['color'] }}">{{ $f['title'] }}</h3>
    ...
    <div class="max-w-4xl mx-auto text-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="bg-white rounded-3xl p-8 shadow-md relative">
                ...
                <div class="mb-4 mt-4" style="font-size:3rem; color:{{ $step['color'] }};"><i class="{{ $step['fa'] }}"></i></div>
                <h3 class="font-fredoka text-2xl mb-3 text-gray-800">{{ $step['title'] }}</h3>
...
<section id="roles" class="py-20 px-6 bg-white">
    <div class="max-w-4xl mx-auto text-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-2xl mx-auto">
            ...
            <div class="rounded-3xl p-8 text-left shadow-sm" style="background: {{ $role['bg'] }}; border: 2px solid {{ $role['color'] }}30;">
                <div class="mb-4" style="font-size:3rem; color:{{ $role['color'] }};"><i class="{{ $role['fa'] }}"></i></div>
                <h3 class="font-fredoka text-2xl mb-4" style="color: {{ $role['color'] }}">{{ $role['title'] }}</h3>
                <ul class="space-y-2">
                    @foreach($role['items'] as $item)
                    <li class="flex items-center gap-2 text-gray-600 font-semibold">
                        <i class="fa-solid fa-check" style="color: {{ $role['color'] }}"></i> {{ $item }}
                    </li>
                    @endforeach
                </ul>
            </div>
...
<footer class="bg-gray-800 text-gray-400 py-8 px-6 text-center">
    ...
    <p class="text-sm">Making learning fun for every child. &copy; {{ date('Y') }} KinderLearn.</p>
</footer>
