...
<style>:root{--mod-color:{{ $modColor }};--mod-rgb:{{ $modRgb }};}</style>
...
<div class="mb-5">
    <a href="{{ route('student.modules.show', $activity->module_id) }}"
       class="inline-flex items-center gap-1 text-gray-500 font-bold hover:text-gray-800 transition">
        <i class="ri-arrow-left-line"></i> Back to {{ $activity->module?->title ?? 'Module' }}
    </a>
</div>
...
@endphp
    <div style="font-size:3rem; color:{{ $actCol }};"><i class="fa-solid {{ $actIco }}"></i></div>
    <div class="flex-1">
        <h1 class="font-fredoka text-2xl text-gray-800 leading-tight">{{ $activity->title }}</h1>
        <p class="text-gray-400 text-sm mt-0.5">{{ $activity->module?->title ?? 'Module' }}
            @if($submission)
                &nbsp;·&nbsp;
                <span class="text-green-600 font-bold"><i class="fa-solid fa-circle-check"></i> Done · {{ $submission->score }}%</span>
            @endif
        </p>
    </div>
    @if($submission)
        <div class="flex gap-0.5">
            @for($s = 1; $s <= 3; $s++)
                <i class="{{ $s <= $submission->stars_earned ? 'fa-solid' : 'fa-regular' }} fa-star" style="font-size:1.5rem; color:#FFD700;"></i>
            @endfor
        </div>
    @endif
</div>
...
{{-- ============================================================
     RESULTS SCREEN — shown when the student already completed
     ============================================================ --}}
@if($submission)

@if($activity->type === 'tracing' && $submission->stars_earned == 0)
{{-- Tracing: submitted but not yet graded by teacher --}}
<div class="game-card text-center celebrate">
    ...
    <div class="bg-purple-50 border border-purple-200 rounded-2xl px-5 py-3 mb-4 text-left">
        ...
        <p class="text-gray-700 font-semibold">{{ $submission->feedback }}</p>
    </div>
    @endif
    <a href="{{ route('student.modules.show', $activity->module_id) }}"
...
<div class="game-card text-center celebrate">
    ...
    <div class="font-fredoka text-5xl mb-2" style="color:var(--mod-color)">{{ $submission->score }}%</div>
    <div class="font-bold text-gray-600 mb-4 text-lg">
        You earned {{ $submission->stars_earned }} <i class="fa-solid fa-star" style="color:#FFD700;"></i>
        @if($submission->score >= 90) — Amazing!
        @elseif($submission->score >= 60) — Great job!
...
{{-- ============================================================
     MATCHING GAME
     ============================================================ --}}
@elseif($style === 'matching' && !empty($activity->content['pairs']))
...
<div class="game-card">
    ...
    <div id="match-score-bar" class="flex items-center gap-3 mb-5">
        ...
        <span id="match-count" class="text-sm font-bold text-gray-500">0 / {{ count($pairs) }}</span>
    </div>

    <div class="grid grid-cols-2 gap-4">
        <div class="flex flex-col gap-3" id="left-col">
            @foreach($lefts as $i => $left)
            <button class="match-card" data-side="left" data-index="{{ $i }}" data-value="{{ $left }}"
                    onclick="matchClick(this)">{{ $left }}</button>
            @endforeach
        </div>
        <div class="flex flex-col gap-3" id="right-col">
            @foreach($rights as $i => $right)
            <button class="match-card" data-side="right" data-index="{{ $i }}" data-value="{{ $right }}"
                    onclick="matchClick(this)">{{ $right }}</button>
            @endforeach
        </div>
    </div>
</div>

<form id="match-form" method="POST" action="{{ route('student.activities.submit', $activity->id) }}" class="hidden">
    @csrf
    @foreach($questions as $i => $q)
        <input type="hidden" name="answers[{{ $i }}]" id="match-ans-{{ $i }}" value="">
    @endforeach
</form>
...
{{-- ============================================================
     WORD BUILDER GAME
     ============================================================ --}}
@elseif($style === 'word_builder' && !empty($activity->content['word']))
...
<div class="game-card text-center">
    ...
    <div class="flex flex-wrap justify-center gap-3 mb-6" id="wb-letters">
        @foreach($letters as $li => $letter)
            <button class="wb-letter" data-index="{{ $li }}" data-letter="{{ $letter }}"
                    onclick="wbPick(this)">{{ $letter }}</button>
        @endforeach
    </div>
...
<form id="word-form" method="POST" action="{{ route('student.activities.submit', $activity->id) }}" class="hidden">
    @csrf
    <input type="hidden" name="answers[0]" id="wb-answer" value="">
</form>

{{-- ============================================================
     TRACING ACTIVITY — canvas drawing tool
     ============================================================ --}}
@elseif($activity->type === 'tracing' && !empty($activity->file_path) && !$submission)

@php $templateUrl = asset('storage/' . $activity->file_path); @endphp
...
<div class="game-card">
    ...
    <div class="relative rounded-2xl overflow-hidden border-2 border-gray-200 mb-4" id="canvas-wrap"
        ...
        {{-- Template image layer (background, not interactive) --}}
        ...
        {{-- Transparent spacer that gives the div its height --}}
        <img src="{{ $templateUrl }}" alt="template" class="w-full opacity-0 pointer-events-none" id="template-img">
    </div>
...
<form id="tracing-form" method="POST" action="{{ route('student.activities.tracing.submit', $activity->id) }}" class="hidden">
    @csrf
...
<div id="quiz-wrap" class="game-card">
    ...
    <div class="flex items-center gap-3 mb-5">
        ...
        <span id="q-counter" class="text-sm font-bold text-gray-500">1 / {{ count($questions) }}</span>
    </div>

    {{-- Quiz image (shown when a local image matches the question) --}}
...
<form id="quiz-form" method="POST" action="{{ route('student.activities.submit', $activity->id) }}" class="hidden">
    @csrf
    @foreach($questions as $i => $q)
        <input type="hidden" name="answers[{{ $i }}]" id="q-ans-{{ $i }}" value="">
    @endforeach
</form>
...
{{-- ============================================================
     VIDEO
     ============================================================ --}}
@elseif($activity->type === 'video' && !empty($activity->content['video_url']))
...
<div class="game-card">
    ...
    <div class="rounded-2xl overflow-hidden mb-5" style="aspect-ratio:16/9;">
        <iframe src="{{ $activity->content['video_url'] }}" class="w-full h-full"
                frameborder="0" allowfullscreen></iframe>
    </div>
    <div class="text-center">
        <form method="POST" action="{{ route('student.activities.submit', $activity->id) }}">
            @csrf
            <button type="submit"
...
var CURRENT_MODULE     = '{{ $activity->module?->title ?? '' }}';
...
(function () {
    ...
    const questions  = @json($questions);
    const correctMap = @json($correct_map ?? []);
...
(function () {
    ...
    const word    = @json($activity->content['word'] ?? '');
